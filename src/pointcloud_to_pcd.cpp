  ...
  public:
      ...
      cloud_cb (const pcl::PCLPointCloud2::ConstPtr& cloud)
    {
      ...
      if (!fixed_frame_.empty ()) {
        ...
        if (!tf_buffer_.canTransform (fixed_frame_, target_frame, pcl_conversions::fromPCL (cloud->header.stamp), ros::Duration (3.0))) {
          ROS_WARN("Could not get transform!");
          return;
        }

        Eigen::Affine3d transform;
        transform = tf2::transformToEigen (tf_buffer_.lookupTransform (fixed_frame_, target_frame,  pcl_conversions::fromPCL (cloud->header.stamp)));
